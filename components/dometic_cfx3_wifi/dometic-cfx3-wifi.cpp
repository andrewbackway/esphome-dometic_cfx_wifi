...
static const Topic TOPICS[] = {
    ...
    // History
    {0, 64, 1, 1, "COMPARTMENT_0_TEMPERATURE_HISTORY_HOUR",
     "HISTORY_DATA_ARRAY"},
    {16, 64, 1, 1, "COMPARTMENT_1_TEMPERATURE_HISTORY_HOUR",
     "HISTORY_DATA_ARRAY"},
    {0, 65, 1, 1, "COMPARTMENT_0_TEMPERATURE_HISTORY_DAY",
     "HISTORY_DATA_ARRAY"},
    {16, 65, 1, 1, "COMPARTMENT_1_TEMPERATURE_HISTORY_DAY",
     "HISTORY_DATA_ARRAY"},
    {0, 66, 1, 1, "COMPARTMENT_0_TEMPERATURE_HISTORY_WEEK",
     "HISTORY_DATA_ARRAY"},
    {16, 66, 1, 1, "COMPARTMENT_1_TEMPERATURE_HISTORY_WEEK",
     "HISTORY_DATA_ARRAY"},
...
void DometicCFXComponent::setup() {
  ESP_LOGI(TAG, "Starting CFX3 component: %s:%u", host_.c_str(),
           (unsigned)port_);
  ...
  while (true) {
    if (this->sock_ < 0) {
      if (!this->connect_task_()) {
        ...
        ESP_LOGI(TAG, "Connection succeeded, waiting for NOP");
      }
    }
...
void DometicCFXComponent::poll_recv_() {
  ...
  while (true) {
    ...
    std::string* line =
        new std::string(this->rxbuf_.substr(start, pos - start));
...
bool DometicCFXComponent::handle_payload_inline_(const std::string& line) {
  ...
  // During handshake we don't want to ACK back from loop; handle_payload_()
  // will send ACK itself.
...
bool DometicCFXComponent::send_json_(const std::string& json) {
  ...
  // ESP_LOGD(TAG, "Sending JSON (2) (size=%u): %s", framed.size(),
  // framed.c_str());

  bool ok = false;
  if (this->send_mutex_ &&
      xSemaphoreTake(this->send_mutex_, pdMS_TO_TICKS(200)) == pdTRUE) {
...
void DometicCFXComponent::publish_bool_(binary_sensor::BinarySensor* b,
                                        bool v) {
  ESP_LOGD(TAG, "Publishing bool: %s = %s",
           b ? b->get_name().c_str() : "(null)", v ? "true" : "false");
  if (b) b->publish_state(v);
}
void DometicCFXComponent::publish_float_(sensor::Sensor* s, float v) {
  ESP_LOGD(TAG, "Publishing float: %s = %f",
           s ? s->get_name().c_str() : "(null)", v);
  if (s) s->publish_state(v);
}
void DometicCFXComponent::publish_text_(text_sensor::TextSensor* t,
                                        const std::string& v) {
  ESP_LOGD(TAG, "Publishing text: %s = %s",
           t ? t->get_name().c_str() : "(null)", v.c_str());
...
bool DometicCFXComponent::handle_payload_(const std::string& line) {
  ...
  if (code == PUBLISH) {
    ...
    auto topic_is = [&](int ta, int tb, int tc, int td) {
      return ta == a && tb == b && tc == c && td == d;
    };
    auto decode_bool = [&]() -> bool { return !val.empty() && (val[0] != 0); };
    auto decode_int8 = [&]() -> float {
      return val.empty() ? NAN : (float)(int8_t)val[0];
    };
    auto decode_uint8 = [&]() -> float {
      return val.empty() ? NAN : (float)(uint8_t)val[0];
    };
    ...
    if (topic_is(0, 1, 1, 1))
      publish_float_(comp0_temp, decode_temp());
    else if (topic_is(16, 1, 1, 1))
      publish_float_(comp1_temp, decode_temp());
    else if (topic_is(0, 2, 1, 1))
      publish_float_(comp0_set_temp, decode_temp());
    else if (topic_is(16, 2, 1, 1))
      publish_float_(comp1_set_temp, decode_temp());
    else if (topic_is(0, 8, 1, 1)) {
      bool cur = decode_bool();
      publish_bool_(comp0_door_open, cur);
      if (cur && !comp0_door_prev_ && comp0_open_count)
        comp0_open_count->publish_state((comp0_open_count->state) + 1.0f);
      comp0_door_prev_ = cur;
    } else if (topic_is(16, 8, 1, 1)) {
      bool cur = decode_bool();
      publish_bool_(comp1_door_open, cur);
      if (cur && !comp1_door_prev_ && comp1_open_count)
        comp1_open_count->publish_state((comp1_open_count->state) + 1.0f);
      comp1_door_prev_ = cur;
    } else if (topic_is(0, 0, 3, 1))
      publish_bool_(cooler_power, decode_bool());
    else if (topic_is(0, 1, 3, 1))
      publish_float_(dc_voltage, decode_deci());
    else if (topic_is(0, 2, 3, 1))
      publish_float_(battery_protection_level, decode_uint8());
    else if (topic_is(0, 5, 3, 1))
      publish_float_(power_source, decode_int8());
    else if (topic_is(0, 6, 3, 1))
      publish_bool_(icemaker_power, decode_bool());
    else if (topic_is(0, 0, 1, 1))
      publish_bool_(comp0_power, decode_bool());
    else if (topic_is(16, 0, 1, 1))
      publish_bool_(comp1_power, decode_bool());
    else if (topic_is(0, 128, 0, 1))
      publish_float_(compartment_count, decode_uint8());
    else if (topic_is(0, 129, 0, 1))
      publish_float_(icemaker_count, decode_uint8());
    else if (topic_is(0, 0, 2, 1))
      publish_float_(presented_temp_unit, decode_uint8());
    else if (topic_is(0, 3, 4, 1))
      publish_bool_(err_comm_alarm, decode_bool());
    else if (topic_is(0, 1, 4, 1))
      publish_bool_(err_ntc_open_large, decode_bool());
    else if (topic_is(0, 2, 4, 1))
      publish_bool_(err_ntc_short_large, decode_bool());
    else if (topic_is(0, 9, 4, 1))
      publish_bool_(err_solenoid_valve, decode_bool());
    else if (topic_is(0, 17, 4, 1))
      publish_bool_(err_ntc_open_small, decode_bool());
    else if (topic_is(0, 18, 4, 1))
      publish_bool_(err_ntc_short_small, decode_bool());
    else if (topic_is(0, 50, 4, 1))
      publish_bool_(err_fan_overvoltage, decode_bool());
    else if (topic_is(0, 51, 4, 1))
      publish_bool_(err_compressor_start_fail, decode_bool());
    else if (topic_is(0, 52, 4, 1))
      publish_bool_(err_compressor_speed, decode_bool());
    else if (topic_is(0, 53, 4, 1))
      publish_bool_(err_controller_overtemp, decode_bool());
    else if (topic_is(0, 3, 5, 1))
      publish_bool_(alert_temp_dcm, decode_bool());
    else if (topic_is(0, 0, 5, 1))
      publish_bool_(alert_temp_cc, decode_bool());
    else if (topic_is(0, 1, 5, 1))
      publish_bool_(alert_door, decode_bool());
    else if (topic_is(0, 2, 5, 1))
      publish_bool_(alert_voltage, decode_bool());
    else if (topic_is(0, 1, 6, 1))
      publish_bool_(wifi_mode, decode_bool());
    else if (topic_is(0, 3, 6, 1))
      publish_bool_(bluetooth_mode, decode_bool());
    else if (topic_is(0, 8, 6, 1))
      publish_bool_(wifi_ap_connected, decode_bool());
    else if (topic_is(0, 193, 0, 0))
      publish_text_(product_serial, std::string(val.begin(), val.end()));
    else if (topic_is(0, 0, 6, 1))
      publish_text_(device_name, std::string(val.begin(), val.end()));
    else if (topic_is(0, 129, 1, 1))
      publish_text_(comp0_recommended_range, to_json_array_(val));
    else if (topic_is(16, 129, 1, 1))
      publish_text_(comp1_recommended_range, to_json_array_(val));
    else if (topic_is(0, 128, 1, 1))
      publish_text_(comp0_temp_range, to_json_array_(val));
    else if (topic_is(16, 128, 1, 1))
      publish_text_(comp1_temp_range, to_json_array_(val));
    else if (topic_is(0, 0, 7, 1))
      publish_text_(station_ssid_0, std::string(val.begin(), val.end()));
    else if (topic_is(1, 0, 7, 1))
      publish_text_(station_ssid_1, std::string(val.begin(), val.end()));
    else if (topic_is(2, 0, 7, 1))
      publish_text_(station_ssid_2, std::string(val.begin(), val.end()));
    else if (topic_is(0, 1, 7, 1))
      publish_text_(station_password_0, std::string(val.begin(), val.end()));
    else if (topic_is(1, 1, 7, 1))
      publish_text_(station_password_1, std::string(val.begin(), val.end()));
    else if (topic_is(2, 1, 7, 1))
      publish_text_(station_password_2, std::string(val.begin(), val.end()));
    else if (topic_is(3, 1, 7, 1))
      publish_text_(station_password_3, std::string(val.begin(), val.end()));
    else if (topic_is(4, 1, 7, 1))
      publish_text_(station_password_4, std::string(val.begin(), val.end()));
    else if (topic_is(0, 2, 7, 1))
      publish_text_(cfx_direct_password_0, std::string(val.begin(), val.end()));
    else if (topic_is(1, 2, 7, 1))
      publish_text_(cfx_direct_password_1, std::string(val.begin(), val.end()));
    else if (topic_is(2, 2, 7, 1))
      publish_text_(cfx_direct_password_2, std::string(val.begin(), val.end()));
    else if (topic_is(3, 2, 7, 1))
      publish_text_(cfx_direct_password_3, std::string(val.begin(), val.end()));
    else if (topic_is(4, 2, 7, 1))
      publish_text_(cfx_direct_password_4, std::string(val.begin(), val.end()));
    else if (topic_is(0, 64, 1, 1)) {
      auto p = decode_history();
      publish_float_(comp0_hist_hour_latest, p.first);
      publish_text_(comp0_hist_hour_json, p.second);
    } else if (topic_is(16, 64, 1, 1)) {
      auto p = decode_history();
      publish_float_(comp1_hist_hour_latest, p.first);
      publish_text_(comp1_hist_hour_json, p.second);
    } else if (topic_is(0, 65, 1, 1)) {
      auto p = decode_history();
      publish_text_(comp0_hist_day_json, p.second);
    } else if (topic_is(16, 65, 1, 1)) {
      auto p = decode_history();
      publish_text_(comp1_hist_day_json, p.second);
    } else if (topic_is(0, 66, 1, 1)) {
      auto p = decode_history();
      publish_text_(comp0_hist_week_json, p.second);
    } else if (topic_is(16, 66, 1, 1)) {
      auto p = decode_history();
      publish_text_(comp1_hist_week_json, p.second);
    } else if (topic_is(0, 64, 3, 1)) {
      auto p = decode_history();
      publish_float_(dc_current_hist_hour_latest, p.first);
      publish_text_(dc_current_hist_hour_json, p.second);
    } else if (topic_is(0, 65, 3, 1)) {
      auto p = decode_history();
      publish_text_(dc_current_hist_day_json, p.second);
    } else if (topic_is(0, 66, 3, 1)) {
      auto p = decode_history();
      publish_text_(dc_current_hist_week_json, p.second);
    } else {
...
// === Float sensors ===
void DometicCFXComponent::set_comp0_temp(sensor::Sensor* sensor) {
  comp0_temp = sensor;
}
void DometicCFXComponent::set_comp1_temp(sensor::Sensor* sensor) {
  comp1_temp = sensor;
}
void DometicCFXComponent::set_comp0_set_temp(sensor::Sensor* sensor) {
  comp0_set_temp = sensor;
}
void DometicCFXComponent::set_comp1_set_temp(sensor::Sensor* sensor) {
  comp1_set_temp = sensor;
}

void DometicCFXComponent::set_dc_voltage(sensor::Sensor* sensor) {
  dc_voltage = sensor;
}
void DometicCFXComponent::set_battery_protection_level(sensor::Sensor* sensor) {
  battery_protection_level = sensor;
}
void DometicCFXComponent::set_power_source(sensor::Sensor* sensor) {
  power_source = sensor;
}
void DometicCFXComponent::set_compartment_count(sensor::Sensor* sensor) {
  compartment_count = sensor;
}
void DometicCFXComponent::set_icemaker_count(sensor::Sensor* sensor) {
  icemaker_count = sensor;
}

void DometicCFXComponent::set_comp0_hist_hour_latest(sensor::Sensor* sensor) {
  comp0_hist_hour_latest = sensor;
}
void DometicCFXComponent::set_comp1_hist_hour_latest(sensor::Sensor* sensor) {
  comp1_hist_hour_latest = sensor;
}
void DometicCFXComponent::set_comp0_hist_day_latest(sensor::Sensor* sensor) {
  comp0_hist_day_latest = sensor;
}
void DometicCFXComponent::set_comp1_hist_day_latest(sensor::Sensor* sensor) {
  comp1_hist_day_latest = sensor;
}
void DometicCFXComponent::set_comp0_hist_week_latest(sensor::Sensor* sensor) {
  comp0_hist_week_latest = sensor;
}
void DometicCFXComponent::set_comp1_hist_week_latest(sensor::Sensor* sensor) {
  comp1_hist_week_latest = sensor;
}
void DometicCFXComponent::set_dc_current_hist_hour_latest(
    sensor::Sensor* sensor) {
  dc_current_hist_hour_latest = sensor;
}

void DometicCFXComponent::set_comp0_open_count(sensor::Sensor* sensor) {
  comp0_open_count = sensor;
}
void DometicCFXComponent::set_comp1_open_count(sensor::Sensor* sensor) {
  comp1_open_count = sensor;
}

void DometicCFXComponent::set_presented_temp_unit(sensor::Sensor* sensor) {
  presented_temp_unit = sensor;
}

// === Binary sensors ===
void DometicCFXComponent::set_cooler_power(
    binary_sensor::BinarySensor* sensor) {
  cooler_power = sensor;
}
void DometicCFXComponent::set_comp0_power(binary_sensor::BinarySensor* sensor) {
  comp0_power = sensor;
}
void DometicCFXComponent::set_comp1_power(binary_sensor::BinarySensor* sensor) {
  comp1_power = sensor;
}
void DometicCFXComponent::set_comp0_door_open(
    binary_sensor::BinarySensor* sensor) {
  comp0_door_open = sensor;
}
void DometicCFXComponent::set_comp1_door_open(
    binary_sensor::BinarySensor* sensor) {
  comp1_door_open = sensor;
}
void DometicCFXComponent::set_icemaker_power(
    binary_sensor::BinarySensor* sensor) {
  icemaker_power = sensor;
}
void DometicCFXComponent::set_wifi_mode(binary_sensor::BinarySensor* sensor) {
  wifi_mode = sensor;
}
void DometicCFXComponent::set_bluetooth_mode(
    binary_sensor::BinarySensor* sensor) {
  bluetooth_mode = sensor;
}
void DometicCFXComponent::set_wifi_ap_connected(
    binary_sensor::BinarySensor* sensor) {
  wifi_ap_connected = sensor;
}

void DometicCFXComponent::set_err_comm_alarm(
    binary_sensor::BinarySensor* sensor) {
  err_comm_alarm = sensor;
}
void DometicCFXComponent::set_err_ntc_open_large(
    binary_sensor::BinarySensor* sensor) {
  err_ntc_open_large = sensor;
}
void DometicCFXComponent::set_err_ntc_short_large(
    binary_sensor::BinarySensor* sensor) {
  err_ntc_short_large = sensor;
}
void DometicCFXComponent::set_err_solenoid_valve(
    binary_sensor::BinarySensor* sensor) {
  err_solenoid_valve = sensor;
}
void DometicCFXComponent::set_err_ntc_open_small(
    binary_sensor::BinarySensor* sensor) {
  err_ntc_open_small = sensor;
}
void DometicCFXComponent::set_err_ntc_short_small(
    binary_sensor::BinarySensor* sensor) {
  err_ntc_short_small = sensor;
}
void DometicCFXComponent::set_err_fan_overvoltage(
    binary_sensor::BinarySensor* sensor) {
  err_fan_overvoltage = sensor;
}
void DometicCFXComponent::set_err_compressor_start_fail(
    binary_sensor::BinarySensor* sensor) {
  err_compressor_start_fail = sensor;
}
void DometicCFXComponent::set_err_compressor_speed(
    binary_sensor::BinarySensor* sensor) {
  err_compressor_speed = sensor;
}
void DometicCFXComponent::set_err_controller_overtemp(
    binary_sensor::BinarySensor* sensor) {
  err_controller_overtemp = sensor;
}

void DometicCFXComponent::set_alert_temp_dcm(
    binary_sensor::BinarySensor* sensor) {
  alert_temp_dcm = sensor;
}
void DometicCFXComponent::set_alert_temp_cc(
    binary_sensor::BinarySensor* sensor) {
  alert_temp_cc = sensor;
}
void DometicCFXComponent::set_alert_door(binary_sensor::BinarySensor* sensor) {
  alert_door = sensor;
}
void DometicCFXComponent::set_alert_voltage(
    binary_sensor::BinarySensor* sensor) {
  alert_voltage = sensor;
}

// === Text sensors ===
void DometicCFXComponent::set_product_serial(text_sensor::TextSensor* sensor) {
  product_serial = sensor;
}
void DometicCFXComponent::set_device_name(text_sensor::TextSensor* sensor) {
  device_name = sensor;
}

void DometicCFXComponent::set_comp0_recommended_range(
    text_sensor::TextSensor* sensor) {
  comp0_recommended_range = sensor;
}
void DometicCFXComponent::set_comp1_recommended_range(
    text_sensor::TextSensor* sensor) {
  comp1_recommended_range = sensor;
}
void DometicCFXComponent::set_comp0_temp_range(
    text_sensor::TextSensor* sensor) {
  comp0_temp_range = sensor;
}
void DometicCFXComponent::set_comp1_temp_range(
    text_sensor::TextSensor* sensor) {
  comp1_temp_range = sensor;
}

void DometicCFXComponent::set_comp0_hist_hour_json(
    text_sensor::TextSensor* sensor) {
  comp0_hist_hour_json = sensor;
}
void DometicCFXComponent::set_comp1_hist_hour_json(
    text_sensor::TextSensor* sensor) {
  comp1_hist_hour_json = sensor;
}
void DometicCFXComponent::set_comp0_hist_day_json(
    text_sensor::TextSensor* sensor) {
  comp0_hist_day_json = sensor;
}
void DometicCFXComponent::set_comp1_hist_day_json(
    text_sensor::TextSensor* sensor) {
  comp1_hist_day_json = sensor;
}
void DometicCFXComponent::set_comp0_hist_week_json(
    text_sensor::TextSensor* sensor) {
  comp0_hist_week_json = sensor;
}
void DometicCFXComponent::set_comp1_hist_week_json(
    text_sensor::TextSensor* sensor) {
  comp1_hist_week_json = sensor;
}
void DometicCFXComponent::set_dc_current_hist_hour_json(
    text_sensor::TextSensor* sensor) {
  dc_current_hist_hour_json = sensor;
}
void DometicCFXComponent::set_dc_current_hist_day_json(
    text_sensor::TextSensor* sensor) {
  dc_current_hist_day_json = sensor;
}
void DometicCFXComponent::set_dc_current_hist_week_json(
    text_sensor::TextSensor* sensor) {
  dc_current_hist_week_json = sensor;
}

void DometicCFXComponent::set_station_ssid_0(text_sensor::TextSensor* sensor) {
  station_ssid_0 = sensor;
}
void DometicCFXComponent::set_station_ssid_1(text_sensor::TextSensor* sensor) {
  station_ssid_1 = sensor;
}
void DometicCFXComponent::set_station_ssid_2(text_sensor::TextSensor* sensor) {
  station_ssid_2 = sensor;
}

void DometicCFXComponent::set_station_password_0(
    text_sensor::TextSensor* sensor) {
  station_password_0 = sensor;
}
void DometicCFXComponent::set_station_password_1(
    text_sensor::TextSensor* sensor) {
  station_password_1 = sensor;
}
void DometicCFXComponent::set_station_password_2(
    text_sensor::TextSensor* sensor) {
  station_password_2 = sensor;
}
void DometicCFXComponent::set_station_password_3(
    text_sensor::TextSensor* sensor) {
  station_password_3 = sensor;
}
void DometicCFXComponent::set_station_password_4(
    text_sensor::TextSensor* sensor) {
  station_password_4 = sensor;
}

void DometicCFXComponent::set_cfx_direct_password_0(
    text_sensor::TextSensor* sensor) {
  cfx_direct_password_0 = sensor;
}
void DometicCFXComponent::set_cfx_direct_password_1(
    text_sensor::TextSensor* sensor) {
  cfx_direct_password_1 = sensor;
}
void DometicCFXComponent::set_cfx_direct_password_2(
    text_sensor::TextSensor* sensor) {
  cfx_direct_password_2 = sensor;
}
void DometicCFXComponent::set_cfx_direct_password_3(
    text_sensor::TextSensor* sensor) {
  cfx_direct_password_3 = sensor;
}
void DometicCFXComponent::set_cfx_direct_password_4(
    text_sensor::TextSensor* sensor) {
  cfx_direct_password_4 = sensor;
}
